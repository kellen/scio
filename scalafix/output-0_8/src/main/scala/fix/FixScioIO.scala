...
class SpecializedScioIO extends ScioIO[(String, Int)] {
  ...
  override def read(sc: ScioContext, endpoint: String): SCollection[(String, Int)] = ???

  override protected def write(data: SCollection[(String, Int)], params: WriteP): Tap[tapT.T] =
...
class OtherScioIO extends ScioIO[Integer] {
  ...
  override def write(data: SCollection[Integer], params: WriteP): Tap[tapT.T] =
    throw new IllegalStateException("🤡")
